...
import { EyeClosed, EyeIcon, Loader, PencilIcon } from "lucide-react";
...
const formSchema = z.object({
  password: z.string().min(2, { message: "كلمة المرور مطلوب" }),
});

const EditPasswordForm = ({ initialData, userId }: UsernameFormProps) => {
  ...
  return (
    <div className="mt-6 border bg-slate-100/10 rounded-md p-4 space-y-5">
      ...
      {!isEditing && <p className="text-sm mt-2">{initialData.password}</p>}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex items-center flex-row text-start space-x-3 space-y-0 rounded-md border p-4 relative">
                  ...
                  {isOpen ? (
                    <EyeIcon className="cursor-pointer" onClick={toggleOpen} />
                  ) : (
                    <EyeClosed
